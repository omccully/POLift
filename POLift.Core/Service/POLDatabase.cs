...
namespace POLift.Core.Service
{
    ...
    public class POLDatabase : IPOLDatabase
    {
        ...
        public POLDatabase(ISQLitePlatform platform, string file_path)
        {
            this.FilePath = file_path;

            this.Platform = platform;
            Connection = new SQLiteConnection(platform, file_path);
        ...
        public bool HideDeletable<T>(T obj) where T : class, IDatabaseObject, IDeletable, new()
        {
            obj.Deleted = true;
            if (obj.ID == 0)
            {
        ...
        public bool FillIDValue<T>(T obj) where T : class, IDatabaseObject, IIdentifiable, new()
        {
            try
            {
        ...
        public IEnumerable<T> Table<T>() where T : class, IDatabaseObject, new()
        {
            lock (Locker)
            {
        ...
        public IEnumerable<T> TableWhereUndeleted<T>() where T : class, IDatabaseObject, IDeletable, new()
        {
            lock (Locker)
            {
                /*var tab = Connection.Table<T>();
        ...
        public T Get<T>(object objp) where T : class, new()
        {
            lock (Locker)
            {
        ...
        public T ReadByID<T>(int ID) where T : class, IDatabaseObject, new()
        {
        ...
        public void ImportDatabaseFromFile(ISQLitePlatform platform, string file_path)
        {
            ImportDatabase(new POLDatabase(platform, file_path));
        }
